...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['notification_id'], $_POST['service_type'], $_POST['service_id'])) {
    ...
    switch ($service_type) {
        case 'appointment':
            header("Location: assigned_appointments.php?id=$service_id");
            break;
        case 'consultation':
            header("Location: give_consultation.php?id=$service_id");
            break;
        case 'other':
        default:
            header("Location: doctor_dashboard.php"); // Default redirection
            break;
    }
    exit();
}
